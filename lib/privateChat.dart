...
class _PrivateChatVupy extends State<PrivateChatVupy> {
  ...
  void portToprivate() async{
    if (imageF.length > 0 || publ.text != '') {
      ...
      if (imageF.length > 0) {
        var stream = new http.ByteStream(DelegatingStream.typed(file[0].openRead()));
        var length = await file[0].length();
        var multipartFile = new http.MultipartFile('img', stream, length, filename: basename(file[0].path));
                                ...
                                return GestureDetector(
                                  onTap: (){
                                    showDialog(
                                      ...
                                      builder: (context) {
                                        return AlertDialog(
                                          title: Text("Deseja remover essa imagem antes de enviar.",textAlign: TextAlign.center,),
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                    image: DecorationImage(
                                      image: FileImage(File(imageF[index])),
                                      fit: BoxFit.cover,
                                    ),
                                  ),
                                  ),
